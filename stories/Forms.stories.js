...
const formWithOnFormInit = {
  ...
  events: {
    onFormInit: `function(form, interceptor) {
...
**Async data fetch with interceptor:**
\`\`\`js
function(form, interceptor) {
  interceptor.get('/openmrs/ws/rest/v1/patient/' + form.getPatient().uuid)
    .then(function(response) {
      form.get('Notes').setValue(response.data.display);
    })
    .catch(function(err) {
...
\`\`\`js
// Note: runEventScript is an internal library function and is not exported from the package.
// Triggering the onFormSave script after saving is the consumer application's responsibility.
...
async function handleSave(containerRef, formMetadata, patient) {
  ...
  // 2. Run the onFormSave script if present via your application's script runner
  const script = formMetadata.events && formMetadata.events.onFormSave;
  if (script) {
    yourAppScriptRunner(script, formMetadata, patient);
  }
}
\`\`\`
